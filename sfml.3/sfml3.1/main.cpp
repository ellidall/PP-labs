...
void update(const sf::Vector2f &mousePosition, sf::ConvexShape &pointer, sf::Clock &clock)
{
    ...
    float angleRadians = atan2(delta.y, delta.x);
    float deltaTime = clock.restart().asSeconds();

    std::cout << "rotation = " << rotation << std::endl;
    std::cout << "angle in Radians = " << angleRadians << std::endl;
    float angleDegrees = toDegrees(angleRadians);
    std::cout << "angle in Degrees = " << angleDegrees << std::endl;

    if (angleDegrees > rotation)
    {
        pointer.setRotation(rotation + std::min(angleDegrees - rotation, (float)15) * deltaTime);
    }
    else
    {
        pointer.setRotation(rotation - std::min(rotation - angleDegrees, (float)15) * deltaTime);
    }
}
